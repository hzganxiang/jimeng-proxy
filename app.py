...
=================================
更新内容：
1. 图片生成增加像素尺寸选择(800x800/1440x1440/1440x1920)
2. 图片生成增加以图生图(参考图上传)
3. 视频播放修复(crossorigin/playsinline/错误回退)
4. 视频Tab Seedance 2.0增加图片上传入口
5. 反推提示词视觉模型改为环境变量可配置+友好报错
6. 批量Tab补全像素选项
7. 工作流视频生成修复(失败自动回退纯文生视频)
8. 分辨率参数小写(1k/2k)
9. 图片生成并行化(多张同时生成)
10. 增加下载按钮
11. Loading状态优化(进度条动画)
12. 整体UI微调
13. 错误处理增强
"""
...
def vision(system, user, image_url=None, image_base64=None):
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            if "ModelNotOpen" in err_text or "not activated" in err_text:
                return {"success": False, "error": f"视觉模型 {VISION_MODEL} 未开通，请在火山方舟控制台激活，或设置环境变量 VISION_MODEL 为已开通的模型"}
            return {"success": False, "error": f"HTTP {resp.status_code}: {err_text[:100]}"}
        result = resp.json()
        choices = result.get("choices") or []
        if choices:
            return {"success": True, "content": choices[0].get("message", {}).get("content", "")}
...
def gen_video(prompt, image_url=None, duration=5, model=None, ratio="16:9"):
    try:
        ...
        if image_url:
            ...
            print(f"[视频生成] 图生视频 模型:{use_model}", flush=True)
        else:
            use_model = model or JIMENG_VIDEO_MODEL_TEXT
            if use_model == JIMENG_VIDEO_MODEL_IMAGE:
                use_model = JIMENG_VIDEO_MODEL_TEXT
                print(f"[视频生成] Seedance需要图片，自动切回3.5Pro", flush=True)
            payload = {"model": use_model, "prompt": prompt, "ratio": ratio, "duration": duration}
            print(f"[视频生成] 纯文生视频 模型:{use_model}", flush=True)
...
def api_generate_images():
    ...
    if pixel_size and pixel_size in PIXEL_TO_PARAMS:
        params = PIXEL_TO_PARAMS[pixel_size]
        ratio = params["ratio"]
        resolution = params["resolution"]
    else:
        resolution = resolution.lower()
    ref_imgs = None
    if ref and isinstance(ref, str) and ref.startswith("http"):
        ref_imgs = [ref]
    images = []
    errors = []
    def gen(i):
        return gen_image(prompt, ratio, resolution, ref_images=ref_imgs, strength=strength)
    ...
    if not images:
        return jsonify({"success": False, "error": errors[0] if errors else "所有图片生成失败"})
    return jsonify({"success": True, "images": [x["url"] for x in images], "errors": errors if errors else None})
...
def api_generate_video():
    ...
    if not prompt and not img:
        return jsonify({"success": False, "error": "请提供视频描述或参考图"}), 400
    if not prompt:
...
def api_batch_images():
    ...
    return jsonify({"success": len(images) > 0, "images": [x["url"] for x in images], "total": count, "done": len(images), "failed": len(errors), "errors": errors[:5] if errors else None})
...
def api_image_to_copy():
    ...
    if not img_url and not img_b64:
        return jsonify({"success": False, "error": "请提供图片"}), 400
    styles = {"douyin": "抖音风格：简短有力，带emoji表情，吸引眼球", "xiaohongshu": "小红书风格：亲切种草，带话题标签#", "taobao": "淘宝风格：突出卖点和优惠，引导下单", "formal": "正式风格：专业描述，突出品质"}
    user_prompt = f"分析这张图片，为{'产品【'+product+'】' if product else '图中产品'}生成营销文案。\n风格：{styles.get(style, styles['douyin'])}\n输出：1.主标题 2.副标题 3.正文(50-100字) 4.标签(3-5个)"
    r = vision("你是专业的电商文案师。", user_prompt, image_url=img_url, image_base64=img_b64)
    if r.get("success"):
...
def api_batch_videos():
    ...
    return jsonify({"success": len(videos) > 0, "videos": [{"url": x["url"], "prompt": x.get("prompt")} for x in videos], "total": len(prompts), "done": len(videos), "failed": len(errors)})
...
def api_generate_storyboard():
    ...
    if not name:
        return jsonify({"success": False, "error": "请输入产品名称"}), 400
    prompt = f'为【{name}】设计{count}个视频分镜。\n文案参考：{copy if copy else "无"}\n\n输出严格按照以下JSON格式，不要其他内容：\n{{"scenes":[{{"image_prompt":"详细的图片描述","video_prompt":"视频动作描述"}}]}}'
    r = chat("你是专业的视频分镜师。请严格按JSON格式输出。", prompt)
...
.upload-area.has-image{border-color:#667eea;background:rgba(102,126,234,0.05)}
...
.progress-fill.animated{animation:progressPulse 1.5s ease-in-out infinite}
...
.ref-upload-area{border:2px dashed rgba(255,255,255,0.15);border-radius:10px;padding:15px;text-align:center;cursor:pointer;transition:all .3s;margin-bottom:14px;font-size:13px;color:var(--tm)}
.ref-upload-area:hover{border-color:rgba(255,255,255,0.3)}
.ref-upload-area.has-image{border-color:#667eea;padding:8px}
.ref-upload-area img{max-height:80px;border-radius:6px}
</style>
</head>
<body>
<div class="container">
<div class="header">
<h1>🎨 AI创作工具 <span class="badge">v9.0</span></h1>
...
<div class="form-group">
<label>参考图URL（可选，以图生图）</label>
<input type="text" id="refImageUrl" placeholder="粘贴已有图片URL，基于此图生成新图">
</div>
...
<div style="font-size:12px;color:var(--tm);margin-top:4px">值越大越接近参考图，越小越自由发挥</div>
</div>
<div class="form-row">
<div class="form-group"><label>像素尺寸</label>
<select id="imagePixel">
<option value="800x800">800×800</option>
<option value="1440x1440" selected>1440×1440</option>
...
<div id="imageProgress" class="card hidden">
<div id="imageProgressText">⏳ 正在并行生成图片...</div>
<div class="progress-bar"><div id="imageProgressBar" class="progress-fill animated" style="width:60%"></div></div>
</div>
...
<div id="videoImageUploadGroup" class="hidden">
<div class="form-group">
<label>参考图URL（Seedance 2.0 必须提供）</label>
<input type="text" id="videoImageUrl" placeholder="粘贴图片URL（先在图片Tab生成，右键复制链接）">
<div style="font-size:12px;color:var(--tm);margin-top:4px">💡 先在「图片」Tab生成图片，右键复制图片链接后粘贴到这里</div>
</div>
</div>
<div class="form-row">
<div class="form-group"><label>模型</label>
<select id="videoModel">
<option value="jimeng-video-3.5-pro">3.5 Pro（纯文生视频）</option>
<option value="jimeng-video-seedance-2.0">Seedance 2.0（图生视频）</option>
</select>
</div>
...
<div id="videoProgressText">⏳ 正在生成视频（约1-3分钟）...</div>
<div class="progress-bar"><div id="videoProgressBar" class="progress-fill animated" style="width:30%"></div></div>
</div>
...
<div class="progress-bar"><div id="batchProgressBar" class="progress-fill animated" style="width:0%"></div></div>
</div>
...
<p style="color:var(--tm);font-size:13px;margin-bottom:15px">提供多张图片URL进行融合</p>
<div class="form-group">
...
<div class="progress-bar"><div id="workflowProgressBar" class="progress-fill animated" style="width:0%"></div></div>
</div>
...
function createVideoPlayer(url,ar){
    ar=ar||'16/9';
    var wrap=document.createElement('div');wrap.className='video-wrapper';wrap.style.aspectRatio=ar;
    var v=document.createElement('video');v.src=url;v.controls=true;v.playsInline=true;v.preload='metadata';v.crossOrigin='anonymous';
    v.onerror=function(){
        var e=document.createElement('div');e.className='video-error';
        e.innerHTML='<div>⚠️ 视频加载失败</div><a href="'+url+'" target="_blank" style="color:#667eea;font-size:12px">点击直接打开链接</a>';
        wrap.appendChild(e);
    };
    wrap.appendChild(v);
    var dl=document.createElement('a');dl.href=url;dl.target='_blank';dl.style.cssText='display:block;text-align:center;padding:6px;font-size:12px;color:#667eea;text-decoration:none';dl.textContent='⬇️ 下载视频';
    var c=document.createElement('div');c.appendChild(wrap);c.appendChild(dl);return c;
}

function createImageItem(u){
    return'<div class="result-item"><img src="'+u+'" loading="lazy"><div class="overlay"><a href="'+u+'" target="_blank" class="btn btn-sm" style="text-decoration:none;color:#fff">🔍 查看</a><a href="'+u+'" download class="btn btn-sm" style="text-decoration:none;color:#fff">⬇️ 下载</a></div></div>';
}

var currentStyle='realistic',currentReverseStyle='detailed',currentBatchMode='images',reverseImageData=null,currentTemplate=null;
var STYLES={realistic:'超高清摄影，真实质感',anime:'日系动漫风格',art:'艺术插画',poster:'商业海报设计','3d':'3D渲染'};

$$('#mainTabs .tab').forEach(function(tab){
    tab.onclick=function(){
        $$('#mainTabs .tab').forEach(function(t){t.classList.remove('active')});this.classList.add('active');
        var n=this.dataset.tab;$$('.tab-content').forEach(function(c){c.classList.add('hidden')});$('#'+n+'Tab').classList.remove('hidden');
        if(n==='template')loadTemplates();
    };
});
...
$$('#reverseStyleChips .chip').forEach(function(c){c.onclick=function(){$$('#reverseStyleChips .chip').forEach(function(x){x.classList.remove('active')});this.classList.add('active');currentReverseStyle=this.dataset.style;}});
$$('#batchModeChips .chip').forEach(function(c){c.onclick=function(){$$('#batchModeChips .chip').forEach(function(x){x.classList.remove('active')});this.classList.add('active');currentBatchMode=this.dataset.mode;$('#batchImagesMode').classList.toggle('hidden',currentBatchMode!=='images');$('#batchVideosMode').classList.toggle('hidden',currentBatchMode!=='videos');}});
...
// 视频模型切换
$('#videoModel').onchange=function(){$('#videoImageUploadGroup').classList.toggle('hidden',this.value!=='jimeng-video-seedance-2.0')};

// 反推上传
$('#reverseUpload').onclick=function(){$('#reverseInput').click()};
$('#reverseInput').onchange=function(){if(this.files[0]){var r=new FileReader();r.onload=function(e){reverseImageData=e.target.result;$('#reversePreview').innerHTML='<div class="preview-item" style="width:120px;height:120px"><img src="'+reverseImageData+'"></div>'};r.readAsDataURL(this.files[0])}};

async function api(url,data){try{var r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});if(!r.ok){var t=await r.text();throw new Error('HTTP '+r.status+': '+t.substring(0,100))}return await r.json()}catch(e){return{success:false,error:e.message}}}
...
$('#genImageBtn').onclick=async function(){
    ...
    var payload={prompt:p+'，'+STYLES[currentStyle],count:parseInt($('#imageCount').value),pixel_size:$('#imagePixel').value};
    var ref=$('#refImageUrl').value.trim();
    if(ref){payload.ref_image=ref;payload.strength=parseFloat($('#refStrength').value)}
    var d=await api('/api/generate-images',payload);
    this.disabled=false;this.textContent='🚀 生成图片';$('#imageProgress').classList.add('hidden');
    if(d.success&&d.images&&d.images.length>0){$('#imageGrid').innerHTML=d.images.map(createImageItem).join('');$('#imageResults').classList.remove('hidden')}else{alert('生成失败: '+(d.error||'未知错误'))}
};

// 视频生成
$('#genVideoBtn').onclick=async function(){
    var p=$('#videoPrompt').value.trim(),m=$('#videoModel').value,imgUrl=($('#videoImageUrl')?$('#videoImageUrl').value:'').trim();
    if(m==='jimeng-video-seedance-2.0'&&!imgUrl){alert('Seedance 2.0 需要提供图片URL\n\n请先在「图片」Tab生成图片，右键复制图片链接后粘贴');return}
    if(!p&&!imgUrl){alert('请输入视频描述');return}
    if(!p)p='smooth cinematic movement';
    this.disabled=true;this.textContent='⏳ 生成中...';$('#videoProgress').classList.remove('hidden');$('#videoResults').classList.add('hidden');
    var d=await api('/api/generate-video',{prompt:p,duration:parseInt($('#videoDuration').value),model:m,ratio:$('#videoRatio').value,image_url:imgUrl||undefined});
    this.disabled=false;this.textContent='🚀 生成视频';$('#videoProgress').classList.add('hidden');
    if(d.success&&(d.url||d.video_url)){var url=d.url||d.video_url;$('#videoGrid').innerHTML='';$('#videoGrid').appendChild(createVideoPlayer(url,$('#videoRatio').value.replace(':','/')));$('#videoResults').classList.remove('hidden')}else{alert('生成失败: '+(d.error||'未知错误'))}
};

// 反推
$('#reverseBtn').onclick=async function(){if(!reverseImageData){alert('请上传图片');return}this.disabled=true;this.textContent='⏳ 分析中...';var d=await api('/api/reverse-prompt',{image_base64:reverseImageData,style:currentReverseStyle});this.disabled=false;this.textContent='🔍 分析图片';if(d.success){$('#reversedPrompt').value=d.prompt;$('#reverseResult').classList.remove('hidden')}else{alert('分析失败: '+(d.error||'未知错误'))}};
...
$('#batchImagesBtn').onclick=async function(){
    var p=$('#batchImagePrompt').value.trim();if(!p){alert('请输入提示词');return}
    this.disabled=true;$('#batchProgress').classList.remove('hidden');$('#batchResults').classList.add('hidden');$('#batchTotal').textContent=$('#batchImageCount').value;$('#batchDone').textContent='0';$('#batchFailed').textContent='0';$('#batchProgressBar').style.width='10%';
    ...
    this.disabled=false;$('#batchProgressBar').style.width='100%';$('#batchProgressBar').classList.remove('animated');$('#batchDone').textContent=d.done||0;$('#batchFailed').textContent=d.failed||0;
    if(d.images&&d.images.length>0){$('#batchGrid').innerHTML=d.images.map(createImageItem).join('');$('#batchResults').classList.remove('hidden')}else{alert('生成失败')}
};

// 批量视频
$('#batchVideosBtn').onclick=async function(){
    var ps=$('#batchVideoPrompts').value.trim();if(!ps){alert('请输入提示词');return}
    var prompts=ps.split('\n').filter(function(p){return p.trim()});
    this.disabled=true;$('#batchProgress').classList.remove('hidden');$('#batchTotal').textContent=prompts.length;
    var d=await api('/api/batch-videos',{prompts:prompts,duration:5,ratio:'16:9'});
    this.disabled=false;$('#batchProgressBar').style.width='100%';$('#batchProgressBar').classList.remove('animated');$('#batchDone').textContent=d.done||0;
    if(d.videos&&d.videos.length>0){$('#batchGrid').innerHTML='';d.videos.forEach(function(v){var url=typeof v==='string'?v:v.url;var item=document.createElement('div');item.className='result-item';item.style.aspectRatio='16/9';item.appendChild(createVideoPlayer(url,'16/9'));$('#batchGrid').appendChild(item)});$('#batchResults').classList.remove('hidden')}
};

// 融合
$('#mergeBtn').onclick=async function(){
    var urls=$('#mergeUrls').value.trim().split('\n').filter(function(u){return u.trim().startsWith('http')});if(urls.length<2){alert('请至少提供2个图片URL');return}
    ...
    if(d.success&&(d.url||d.image_url)){$('#mergeGrid').innerHTML=createImageItem(d.url||d.image_url);$('#mergeResult').classList.remove('hidden')}else{alert('融合失败: '+(d.error||'未知错误'))}
};
...
$('#workflowBtn').onclick=async function(){
    ...
    try{
        ...
        if(!cd.success)throw new Error(cd.error||'文案生成失败');var copy=cd.copy;
        ...
        var scenes=(sd.storyboard&&sd.storyboard.scenes)||[];if(scenes.length===0)throw new Error('未生成有效分镜');

        $('#workflowStep').textContent='🖼️ 生成图片...';
        var pairs=[];
        for(var i=0;i<scenes.length;i++){
            $('#workflowProgressBar').style.width=(30+i*20/scenes.length)+'%';$('#workflowStep').textContent='🖼️ 生成图片 '+(i+1)+'/'+scenes.length+'...';
        ...
        if(pairs.length===0)throw new Error('所有图片生成失败');

        $('#workflowStep').textContent='🎬 生成视频...';
        var videos=[];
        for(var i=0;i<pairs.length;i++){
            var pr=pairs[i];$('#workflowProgressBar').style.width=(50+i*45/pairs.length)+'%';$('#workflowStep').textContent='🎬 生成视频 '+(i+1)+'/'+pairs.length+'...';
            ...
            else{var vd2=await api('/api/generate-video',{prompt:pr.scene.video_prompt||pr.scene.image_prompt||'smooth cinematic movement',duration:5,model:'jimeng-video-3.5-pro'});if(vd2.url||vd2.video_url)videos.push(vd2.url||vd2.video_url)}
        }

        $('#workflowProgressBar').style.width='100%';$('#workflowProgressBar').classList.remove('animated');$('#workflowStep').textContent='✅ 完成！';$('#workflowCopy').textContent=copy;
        if(videos.length>0){$('#workflowGrid').innerHTML='';videos.forEach(function(v){var item=document.createElement('div');item.className='result-item';item.style.aspectRatio='16/9';item.appendChild(createVideoPlayer(v,'16/9'));$('#workflowGrid').appendChild(item)})}
        else{$('#workflowGrid').innerHTML='<p style="color:var(--tm)">视频生成失败，但文案和分镜已完成</p>'}
